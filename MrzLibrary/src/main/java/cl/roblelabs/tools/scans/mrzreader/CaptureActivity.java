package cl.roblelabs.tools.scans.mrzreader;
...
import cl.roblelabs.tools.scans.mrzreader.camara.CameraManager;
import cl.roblelabs.tools.scans.mrzreader.interfaces.FinishListener;
import cl.roblelabs.tools.scans.mrzreader.model.MrzResult;
import cl.roblelabs.tools.scans.mrzreader.ocr.OcrCharacterHelper;
import cl.roblelabs.tools.scans.mrzreader.ocr.OcrInitAsyncTask;
import cl.roblelabs.tools.scans.mrzreader.ocr.OcrResult;
import cl.roblelabs.tools.scans.mrzreader.ocr.OcrResultFailure;
import cl.roblelabs.tools.scans.mrzreader.ocr.OcrResultText;
import cl.roblelabs.tools.scans.mrzreader.tasks.DecodeHandler;
import cl.roblelabs.tools.scans.mrzreader.views.ViewfinderView;
...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback
{
    ...
    private void initOcrEngine(File storageRoot, String languageCode, String languageName) {
        ...
        if (ocrEngineMode != TessBaseAPI.OEM_TESSERACT_ONLY) {
            ...
            if (!cubeOk) {
                ...
                prefs.edit().putString(KEY_OCR_ENGINE_MODE, getOcrEngineModeName()).apply();
            }
        }
    ...
    public void handleOcrContinuousDecode(OcrResult ocrResult) {
            ...
            if (ocrResult.getMeanConfidence() >= 60 && textResultTmpArr.length >= 2 && textResultTmpArr.length <= 3) {
                try {
                    ...
                    */
                    ...
                    MrzResult resultado = new MrzResult(ocrResult);
                    if(resultado.validaFormato()){
                    ...
                    Log.w("RobleLabs", "checksum fail", e);
                }
            }
        ...
        if (CONTINUOUS_DISPLAY_METADATA) {
            ...
            statusViewBottom.setText("Calidad de captura: " +  meanConfidence.toString());
        }
    }
    ...
    private void setDefaultPreferences() {
        ...
        // Continuous preview
        prefs.edit().putBoolean(KEY_CONTINUOUS_PREVIEW, CaptureActivity.DEFAULT_TOGGLE_CONTINUOUS).apply();

        // Recognition language
        prefs.edit().putString(KEY_SOURCE_LANGUAGE_PREFERENCE, CaptureActivity.DEFAULT_SOURCE_LANGUAGE_CODE).apply();

        // Translation
        prefs.edit().putBoolean(KEY_TOGGLE_TRANSLATION, CaptureActivity.DEFAULT_TOGGLE_TRANSLATION).apply();

        // Translation target language
        prefs.edit().putString(KEY_TARGET_LANGUAGE_PREFERENCE, CaptureActivity.DEFAULT_TARGET_LANGUAGE_CODE).apply();

        // Translator
        prefs.edit().putString(KEY_TRANSLATOR, CaptureActivity.DEFAULT_TRANSLATOR).apply();

        // OCR Engine
        prefs.edit().putString(KEY_OCR_ENGINE_MODE, CaptureActivity.DEFAULT_OCR_ENGINE_MODE).apply();

        // Autofocus
        prefs.edit().putBoolean(KEY_AUTO_FOCUS, CaptureActivity.DEFAULT_TOGGLE_AUTO_FOCUS).apply();

        // Disable problematic focus modes
        prefs.edit().putBoolean(KEY_DISABLE_CONTINUOUS_FOCUS, CaptureActivity.DEFAULT_DISABLE_CONTINUOUS_FOCUS).apply();

        // Beep
        prefs.edit().putBoolean(KEY_PLAY_BEEP, CaptureActivity.DEFAULT_TOGGLE_BEEP).apply();
        ...
        prefs.edit().putString(KEY_CHARACTER_WHITELIST,
                OcrCharacterHelper.getDefaultWhitelist(DEFAULT_SOURCE_LANGUAGE_CODE)).apply();

        // Page segmentation mode
        prefs.edit().putString(KEY_PAGE_SEGMENTATION_MODE, CaptureActivity.DEFAULT_PAGE_SEGMENTATION_MODE).apply();

        // Reversed camera image
        prefs.edit().putBoolean(KEY_REVERSE_IMAGE, CaptureActivity.DEFAULT_TOGGLE_REVERSED_IMAGE).apply();

        // Light
        prefs.edit().putBoolean(KEY_TOGGLE_LIGHT, CaptureActivity.DEFAULT_TOGGLE_LIGHT).apply();
    }
